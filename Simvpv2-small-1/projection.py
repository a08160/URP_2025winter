...
def get_station_map(image_size, label_type='asos', origin_size=900, input_size=512):
    '''
    관측소의 이미지 좌표를 반환하는 함수

    512x512 또는 384x384 입력 데이터를 기준으로 좌표를 계산합니다.

    Parameters:
        image_size: 타겟 이미지 크기 (192, 384, 512 등)
        label_type: 'asos' or 'aafos'
        origin_size: 원본 이미지 크기 (기본값 900, 2km 해상도)
        input_size: 입력 데이터 크기 (512 또는 384, 기본값 512)
    ...
    Example:
        >>> # 512x512 데이터 사용 시
        >>> asos_map = get_station_map(192, 'asos', input_size=512)
        >>> # 384x384 데이터 사용 시 (이미 crop된 데이터)
        >>> asos_map = get_station_map(192, 'asos', input_size=384)
    '''
    ...
    if input_size == 512:
        if image_size == 512:
            ...
            return {k: (v[0] - offset, v[1] - offset) for k, v in map_512.items()}
    elif input_size == 384:
        if image_size == 384:
            return map_384
        elif image_size == 192:
            # 384 -> 192 resize: 좌표를 2로 나눔
            return {k: (v[0] // 2, v[1] // 2) for k, v in map_384.items()}
        else:
            # 다른 크기로 resize: 비율에 맞게 변환
            ratio = image_size / 384
            return {k: (int(v[0] * ratio), int(v[1] * ratio)) for k, v in map_384.items()}
    else:
        raise ValueError(f"input_size should be 512 or 384, got {input_size}")
