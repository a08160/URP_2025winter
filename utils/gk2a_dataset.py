...
class GK2ADataset(Dataset):
    def __init__(self, data_path:str, output_path:str, data_info_list:list, channels:str, time_range:tuple, channels_calib:list, image_size:int, misc_images:torch.Tensor, patch_size:int=None, patch_candidates:dict=None, transform=None, train:bool=True):
        '''
    ...
    def __getitem__(self, idx):
        ...
        for i, data_info in enumerate(self.data_info_list):
            ...
            if self.misc_images is not None:
                # misc를 각 시점에 추가: (C_misc, H, W) → (T, C_misc, H, W)
                T = images.shape[0]
                misc_expanded = self.misc_images.unsqueeze(0).expand(T, -1, -1, -1)
                images = torch.cat([images, misc_expanded], dim=1)  # (T, C+C_misc, H, W)
            # ================================
