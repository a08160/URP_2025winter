...
class SwiGLU(nn.Module):
    def __init__(
        ...
        super().__init__()
        # x = a or b -> a is not None / False -> a else b
    ...
    def forward(self, x):
        x_gate = self.fc1_g(x)
        x = self.fc1_x(x)
        x = self.act(x_gate) * x # SiLU(x_gate) * x
        x = self.drop1(x)
        x = self.norm(x)
        x = self.fc2(x)
        x = self.drop2(x)
...
class GatedTransformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout=0., attn_dropout=0., drop_path=0.1):
        super().__init__()
        self.layers = nn.ModuleList([]) # 학습 대상 모듈 리스트 선언
        self.norm = nn.LayerNorm(dim)
        for _ in range(depth):
            # PreNorm: norm -> fn == return self.fn(self.norm(x), **kwargs)
...
class PredFormer_Model(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 1, 4, 2, 5, 3, 6).reshape(B, T, self.out_channels, H, W)
                    # B, T, C, num_patches_h, patch_size, num_patches_w, patch_size -> B, T, C, H(), W()
        return x
